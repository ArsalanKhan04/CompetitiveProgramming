...
void ntt(vl &a) {
	...
	for (static int k = 2, s = 2; k < n; k *= 2, s++) {
		...
		FL(i,k,2*k) rt[i] = rt[i / 2] * z[i & 1] % mod;
	}
	vi rev(n);
	FL(i,0,n) rev[i] = (rev[i / 2] | (i & 1) << L) / 2;
	FL(i,0,n) if (i < rev[i]) swap(a[i], a[rev[i]]);
	for (int k = 1; k < n; k *= 2)
		for (int i = 0; i < n; i += 2 * k) FL(j,0,k) {
...
int main() {
  ...
  ios_base::sync_with_stdio(false);
    ...
    while(TCS--){
      ...
      string s;
      cin >> s;
      vi a(n); FL(i,0,n) a[i] = s[i]=='1';
      int zrs = n - accumulate(ALL(a), 0);

      vl A(n + 1);
      vl B(2 * n + 1);

      FL(i,0,n+1) A[i]=(ll) Comb((int)n, (int)i);
      FL(i, 0, 2 * n + 1){
        ll x = abs(i - n);
        B[i] = (x/2)*((x+1)/2);
        B[i] %= md;
